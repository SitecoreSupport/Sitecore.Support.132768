...
namespace Sitecore.Support.Data.SqlServer
{
  public class SqlServerDataProvider : Sitecore.Data.SqlServer.SqlServerDataProvider
  {
    ...
    public override ID ResolvePath(string itemPath, CallContext context)
    {
      ...
      return this.ResolvePathRec(itemPath.Split(new char[] { '/' }, StringSplitOptions.RemoveEmptyEntries), CacheManager.GetPathCache(context.DataManager.Database), context);
    }
    private ID ResolvePathRec([NotNull] string[] path, [NotNull] PathCache cache, [NotNull] CallContext context)
    {
      ...
      // if we have items with the same name under the same parent, we have to sort them and take the first.
    ...
    private bool IsIDPath(string[] path)
    {
      return path.All(itemName => ID.IsID(itemName));
    }
  }
}
